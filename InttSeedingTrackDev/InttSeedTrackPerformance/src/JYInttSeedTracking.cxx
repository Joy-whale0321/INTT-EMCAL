...
Int_t InttSeedTracking::TempINTTIOMatching(Double_t oINTTPhi, Double_t refPhi, std::vector<hitStruct> vIInttHits)
{
...
Double_t InttSeedTracking::TempCalcdPhidR(Int_t iInttID, Int_t oInttID, Double_t refPhi, std::vector<hitStruct> vIInttHits, std::vector<hitStruct> vOInttHits)
{
    Double_t dPhidR = 0.;   
...
Int_t InttSeedTracking::TempInttCalMatch(Int_t iInttID, Int_t oInttID, Double_t refPhi, std::vector<hitStruct> vIInttHits, std::vector<hitStruct> vOInttHits, std::vector<hitStruct> vEmcalHits)
{
...
// loop mvtx ilayer cluster,use delta_R = dR-sagittaR to check is match, get mvtxClusX and mvtxClusY
bool InttSeedTracking::MatchingMvtxHits(Int_t& mvtxMatchId,Double_t& mvtxClusX, Double_t& mvtxClusY,\
   Double_t sagittaR, Double_t centerX, Double_t centerY, std::vector<hitStruct> vMvtxHits)
{
    ...
    // loop mvtx ilayer cluster,use delta_R = dR-sagittaR to check is match? 
    ...
    if(mvtxMatchId == 9999) return false; // no mvtxcluster to match
