...
def train(list_file, pt_min=0.0, pt_max=2.0, batch_size=1024, epochs=200, lr=5e-5, val_ratio=0.3, device="cuda" if torch.cuda.is_available() else "cpu"):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f"model_weight/best_model_pt_{pt_min:.1f}_{pt_max:.1f}.pt")
            print(f"✓ Saved best model (val loss = {val_loss:.4f})")

    print("✅ 训练完成。最优模型保存在 best_model_*.pt")

    joblib.dump(train_set.dataset.scaler, f"model_weight/scaler_pt_{pt_min:.1f}_{pt_max:.1f}.pkl")
    print("✅ 标准化器已保存")
